...
async def fuzz_path_single(session, target_url, path, semaphore):
    ...
    async with semaphore:
        try:
            async with session.get(url, allow_redirects=False, timeout=10) as response:
                ...
                length = len(text)
                return path, url, status, length, text[:150], None
        except Exception as e:
            return path, url, 0, 0, str(e), None

async def run_dir_scan(target_url, wordlist_path, concurrency=20, proxy=None, exclude_statuses=None, exclude_lengths=None):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [
            fuzz_path_single(session, target_url, path, semaphore)
            for path in paths
        ]
        responses = await asyncio.gather(*tasks)
        
    for path, url, status, length, text, err in responses:
